...
* `safe_str(value)`: XML'den gelen veriler boş veya dönüştürülemeyen yapıda olduğunda uygulamanın çökmesini önler ve `input` dan gelen verinin boş veya dönüştürülemeyen yapıda olduğunda uygulamanın çökmesini önler; geçerli bir metin varsa `str` tipine çevirir, veri `None` ise `Deger bos`, metin dışı bir veri ise `Deger none_str` döndürür.
